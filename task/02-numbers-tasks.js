...
function getAverage(value1, value2) {
    ...
    if (isFinite(average))
        return average;
    if (average === Number.NEGATIVE_INFINITY)
...
function roundToPowerOfTen(num, pow) {
    let powOf10 = Math.pow(10, pow);
    return powOf10 * Math.round(num / powOf10);
}
...
function toNumber(value, def) {
    let parseRes = parseFloat(value);
    return isNaN(parseRes) ? def : parseRes;
}
